...
// Note: 'lead', 'dashboard' and 'account' were removed from premium.js — those
// pages are now rendered exclusively by app.js (renderLeadCapturePage,
// renderBusinessDashboardPage and renderAccountPage respectively), because the
// premium.js versions were legacy (localStorage-only wizard / a sidebar
// dashboard that raced the canonical one / an old account view that flashed
// before app.js rendered).
const PREMIUM_PAGES = ['home', 'drinks', 'suppliers', 'venues', 'events', 'product', 'pricing'];
