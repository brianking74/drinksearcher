...
async function renderVenueProfile() {
  ...
  app.innerHTML = `
    ...
    <section class="section-tight"><div class="container grid grid-2"><div class="panel"><span class="eyebrow">Contact & details</span><h3 style="margin:14px 0;">Plan your visit.</h3><div class="muted" style="display:grid; gap:10px;">${claimed ? `${v.phone ? '<span>📞 ' + v.phone + '</span>' : ''}<span>📍 ${v.area || 'Hong Kong'}</span>${v.booking ? '<span>📅 Book via ' + v.booking + '</span>' : ''}` : `<span>Contact details are hidden until this venue is claimed. <a class="text-gold" href="list-your-business.html?type=venue&claim=${slug}">Claim your venue</a> to add them.</span>`}</div></div><div class="panel"><span class="eyebrow">Claim your venue</span><h3 style="margin:14px 0;">Own this venue?</h3><p class="muted">Add direct booking links, imagery, and promoted placement so guests find you first.</p><div class="inline-actions" style="margin-top:18px;"><a class="btn btn-primary" href="list-your-business.html?type=venue&claim=${slug}">Claim your venue</a></div></div></div></section>
...
async function renderSupplierProfile() {
  ...
  app.innerHTML = `
    ...
    <section id="contact" class="section"><div class="container grid grid-2"><div class="panel"><span class="eyebrow">Contact</span><h3 style="margin:14px 0;">Ready to browse or buy?</h3><div class="muted" style="display:grid; gap:10px;">${claimed ? `${phone ? `<span>${phone}</span>` : ''}<span>${area || 'Hong Kong'}</span><span><a href="${website}">${website === '#' ? 'Visit website' : website}</a></span>` : `<span>Contact details are hidden until this listing is claimed. <a class="text-gold" href="list-your-business.html?type=merchant&claim=${slug}">Claim this listing</a> to add them.</span>`}</div></div><div class="panel"><span class="eyebrow">Own this supplier listing?</span><h3 style="margin:14px 0;">Get your profile live</h3><p class="muted">Add your story, catalogue, and store links so shoppers can move from discovery to purchase more easily.</p><div class="inline-actions" style="margin-top:18px;"><a class="btn btn-primary btn-small" href="list-your-business.html?type=merchant&claim=${slug}">Claim this listing</a></div></div></div></section>`;
  bindSaveButtons(app);
}
