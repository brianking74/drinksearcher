  ...
  app.innerHTML = `
    ...
    <section class="section homepage-venues-section">
      <div class="container">
        ...
        <div class="grid grid-4">${featuredVenues.map(v => renderCard(v, {type:'venue', href:`venue-template.html?slug=${v.slug}`, cta:`${ctaLink('View', `venue-template.html?slug=${v.slug}`, 'btn btn-secondary btn-small')}${ctaLink('View', v.website, 'btn btn-ghost btn-small', 'View')}`})).join('')}</div>
      </div>
    </section>
...
async function renderVenueDirectory() {
  ...
  function render() {
    ...
    $('#venue-enhanced').innerHTML = enhanced.length ? enhanced.map(v => renderCard({...v, tierLabel:'Enhanced'}, {type:'venue', href:`venue-template.html?slug=${v.slug}`, cta:`${ctaLink('View', `venue-template.html?slug=${v.slug}`, 'btn btn-secondary btn-small')}${ctaLink('View', v.website, 'btn btn-ghost btn-small', 'Details soon')}`})).join('') : '<div class="empty-state">No enhanced venues match these filters yet.</div>';
